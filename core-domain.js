...
/* =========================
   ...
   ========================= */
// Both game variants use the same 11-stage learning progression.
const LEVEL_ORDER=[...Array.from({length:9},(_,i)=>`block${i+1}`),'blockRandom','blockMixed'];
const MODES={};
for(let n=1;n<=9;n++)MODES[`block${n}`]={name:`第 ${n} 關｜${n} 的乘法`,multipliers:[n],review:'不出加減法',reviewEnabled:false};
MODES.blockRandom={name:'第 10 關｜九九隨機',multipliers:[1,2,3,4,5,6,7,8,9],review:'不出加減法',reviewEnabled:false};
MODES.blockMixed={name:'最終關｜九九＋加減法',multipliers:[1,2,3,4,5,6,7,8,9],review:'兩個數皆 200 以內的加減法',reviewEnabled:true};
...
const game={running:false,paused:false,sound:true,level:'block1',stage:1,score:0,combo:0,correctCount:0,round:0,question:null,phase:'idle',best:Number(storedBest||0),mastery:JSON.parse(storedMastery||'{}')};
...
function levelOrder(){return LEVEL_ORDER}
